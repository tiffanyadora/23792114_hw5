...
/**
 * Loads detailed product information including Pokemon and Weather data
    ...
    fetch(`/api/products/${productId}/`)
        ...
        .then(data => {
            ...
            if (data.success && data.product) {
                // If product has Pokemon data, fetch it directly
                if (data.product.pokemon) {
                    fetch(`/api/pokemon/${data.product.pokemon}/`)
                        ...
                        .catch(error => handlePokemonError('Error loading Pokémon data'));
                } else {
                    handlePokemonError('No Pokémon associated with this product');
                }
                
                // If product has location data, fetch weather directly
                if (data.product.location) {
                    fetch(`/api/weather/${data.product.location}/`)
                        ...
                        .catch(error => handleWeatherError('Error loading weather data'));
                } else {
...
/**
 * Creates or ensures the Pokemon container exists in the DOM
 */
...
function displayPokemonData(pokemonData) {
    ...
    container.innerHTML = `
        ...
        <p class="mt-3">This product features the ${pokemonData.name} Pokémon as its mascot!</p>
    `;
}
    ...
    container.innerHTML = `
        ...
        <div class="d-flex gap-4 align-center">
            <div class="weather-icon">
                ${weatherData.icon ? 
                  `<img src="https://openweathermap.org/img/wn/${weatherData.icon}@2x.png" alt="${weatherData.condition}">` : 
...
// Direct API call functions for frontend use
